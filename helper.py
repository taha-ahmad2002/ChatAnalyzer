...
def daily_timeline(selected_user,df):
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
    daily_timeline_ = df.groupby('only_date').count()['messages'].reset_index()
    return daily_timeline_
...
def activity_heatmap(selected_user,df):
    ...
    activity_heatmap_ = df.pivot_table(index='day_name', columns='period', values='messages', aggfunc='count').fillna(0)
